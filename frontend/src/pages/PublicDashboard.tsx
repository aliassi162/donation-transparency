  ...
  return (
    <Page>
      <section className="rounded-xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-4 shadow-sm sm:p-5">
        <h2 className="text-lg font-semibold text-slate-950 sm:text-xl">{t("aboutTitle")}</h2>
        <p className="mt-2 max-w-4xl text-[15px] leading-7 text-slate-700 sm:text-base">{t("aboutText")}</p>
      </section>
              ...
              {paginatedDonations.map((row, i) => (
                <div key={i} className="rounded-md border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <p className="shrink-0 text-base font-semibold text-emerald-700">{money(row.amount)}</p>
                  </div>
                  <p className="mt-2 text-sm text-slate-500">{row.donor_country || "-"} / {formatDate(row.received_date)}</p>
                </div>
              ))}
            </div>
          ...
          {distributions.length === 0 ? <EmptyState message={t("noDistributionsRecorded")} /> : (
            ...
            <div className="space-y-3 md:hidden">
              {paginatedDistributions.map((row, i) => (
                <div key={`${row.household_code}-${row.distribution_date}-${i}`} className="rounded-md border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-medium text-slate-900">{row.household_code}</p>
                      <p className="mt-2 text-sm leading-5 text-slate-500">{row.location || "-"} / {assistanceTypeLabel(row.assistance_type, t)} / {formatDate(row.distribution_date)}</p>
                    </div>
                    <p className="shrink-0 text-base font-semibold text-sky-700">{money(row.amount)}</p>
                  </div>
                </div>
              ))}
            </div>
...
function Page({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#ecfdf5,transparent_35%),#f8fafc]">
      <div className="mx-auto max-w-6xl space-y-4 px-4 py-4 sm:space-y-5 sm:p-5 md:space-y-6 md:p-8">
      <header className="flex flex-col gap-4 border-b border-emerald-100 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 sm:text-sm">{t("publicDashboard")}</p>
          <h1 className="mt-1 text-[1.7rem] font-semibold leading-tight text-slate-950 sm:text-3xl">{t("publicTitle")}</h1>
          <p className="mt-2 max-w-2xl text-[15px] leading-6 text-slate-600 sm:text-base">{t("transparencyOnly")}</p>
        </div>
