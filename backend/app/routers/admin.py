...
def parse_date(value: str, field: str, errors: list[str]) -> date:
    try:
...
async def import_distributions(file: UploadFile = File(...), db: Session = Depends(get_db)) -> ImportPreview:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if row_errors:
            errors.append(ImportErrorRow(row=index, errors=row_errors))
            continue
        objects.append(Distribution(household_id=households[code], amount=amount, currency="USD", distribution_date=distribution_date, assistance_type=row.get("assistance_type"), notes=row.get("notes")))
    if errors:
        return ImportPreview(valid=False, rows_valid=len(objects), errors=errors)
    db.add_all(objects)
    commit_or_conflict(db)
